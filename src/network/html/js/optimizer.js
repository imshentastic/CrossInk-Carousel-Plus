...
async function showOptimizerPreflightModal(renderInfo, fileName) {
  ...
  // Build the modal DOM once per call, append to body, return a promise
  // that resolves when the user confirms or rejects when they cancel.
  return new Promise((resolve, reject) => {
    ...
    // Form rows. Each one stores its current value in a `data-current` attr
    // so we can detect dirty state on confirm.
    ...
    confirm.onclick = async () => {
      // Collect dirty fields. Only POST what changed -- the server-side
      // handler ignores omitted fields, so we keep the wire payload tiny.
      const updates = {};
      const dirtyKeys = [];
      ...
      // Push updates to the device, then re-fetch a fresh renderInfo so the
      // optimizer's bake uses the same exact values that are now in
      // SETTINGS. The bake's manifest comes from renderInfo so they always
      // line up -- and the device's first cold open after upload sees a
      // fingerprint that matches the cache.
...
async function convertEpubFile(file, progressCallback) {
  ...
  // CrumBLE: preflight modal. When the user has BT or prebake on, the
  // optimization is going to lock the book to a particular reader-settings
  // snapshot for the duration of its existence on the device. Surface that
  // snapshot in a modal so the user can verify (and edit) the values that
  // will be baked in -- and on edit, push the corrected SETTINGS back to
  // the device so the manifest's fingerprint matches what the device's
  // first cold open will compute against. Cancel aborts the whole
  // conversion; throwing here propagates up to convertEpubFile's caller
  // which logs the abort and moves on.
  if (renderInfo) {
    try {
