...
// v18.9.9.441 — CrossInk reading-stats utility parity. Mirrors upstream's
// ReadingStatsUtils 1:1 so per-book / global-stats binary layouts and the
// 730-day streak bitfield are byte-compatible with CrossInk.
...
// Reads the system clock (time()+localtime_r), which is set from DS3231 on X3
// or from SNTP on X4. Returns false if clock is not yet valid.
