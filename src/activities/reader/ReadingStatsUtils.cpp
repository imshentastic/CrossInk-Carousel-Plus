...
// v18.9.9.441 — 1:1 port of CrossInk's ReadingStatsUtils. Differs only in
// getCurrentLocalReadingStatsDateTime which reads the system clock (already
// set from DS3231 on X3 / SNTP on X4) instead of upstream's HalClock combined
// getDateTime + manual UTC-offset apply. Keeps CrumBLE aligned with the same
// pattern our existing ReadingStats.cpp uses.

namespace {
constexpr const char* MONTH_NAMES[] = {"Jan", "Feb", "Mar", "Apr", "May", "Jun",
                                       "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"};

// unix epoch 2020-01-01 UTC. Matches ReadingStats::VALID_CLOCK_THRESHOLD.
...
bool isBitSet(const std::array<uint8_t, READING_HISTORY_BYTES>& bits, const size_t bitIndex) {
  if (bitIndex >= READING_HISTORY_DAYS) return false;
...
void setBit(std::array<uint8_t, READING_HISTORY_BYTES>& bits, const size_t bitIndex) {
  if (bitIndex >= READING_HISTORY_DAYS) return;
...
uint32_t secondsUntilNextBucketBoundary(const ReadingStatsDateTime& dt) {
  ...
  if (nextBoundarySecondOfDay <= currentSecondOfDay) return 1;
...
void addSecondsToReadingStatsDateTime(ReadingStatsDateTime& dt, uint32_t seconds) {
  ...
  if (daysToAdvance > 0) addDaysToReadingStatsDate(dt.date, static_cast<int>(daysToAdvance));
...
uint8_t readingStatsDayOfWeekIndex(const ReadingStatsDate& date) {
  // 2000-01-01 was a Saturday. (5 + dayIndex) % 7 makes Monday = 0.
...
bool getCurrentLocalReadingStatsDateTime(ReadingStatsDateTime& outDateTime) {
  ...
  if (now < static_cast<time_t>(kValidClockThreshold)) {
    outDateTime = {};
    return false;
  }
  struct tm local;
  ...
  outDateTime.date.year = static_cast<uint16_t>(1900 + local.tm_year);
  outDateTime.date.month = static_cast<uint8_t>(local.tm_mon + 1);
  outDateTime.date.day = static_cast<uint8_t>(local.tm_mday);
  outDateTime.hour = static_cast<uint8_t>(local.tm_hour);
  outDateTime.minute = static_cast<uint8_t>(local.tm_min);
  outDateTime.second = static_cast<uint8_t>(local.tm_sec);
...
void recordReadingSpanIntoBuckets(std::array<uint32_t, READING_TIME_BUCKET_COUNT>& timeOfDaySeconds,
  ...
  while (remaining > 0) {
    ...
    const uint32_t boundary = secondsUntilNextBucketBoundary(cursor);
    const uint32_t segment = remaining < boundary ? remaining : boundary;
...
void markReadingHistoryDay(uint32_t& anchorDay, std::array<uint8_t, READING_HISTORY_BYTES>& bits,
  ...
  if (delta >= READING_HISTORY_DAYS) return;
...
uint16_t computeReadingHistoryCurrentStreak(uint32_t anchorDay, const std::array<uint8_t, READING_HISTORY_BYTES>& bits,
  ...
  if (anchorDay == 0 && !isBitSet(bits, 0)) return 0;
  if (!isBitSet(bits, 0)) return 0;
  if (today && today->isValid()) {
    const uint32_t todayDay = readingStatsDayIndex(*today);
    if (anchorDay + 1u < todayDay) return 0;
  }
  uint16_t streak = 0;
  for (size_t bitIndex = 0; bitIndex < READING_HISTORY_DAYS; ++bitIndex) {
    if (!isBitSet(bits, bitIndex)) break;
    streak++;
  }
  return streak;
}
