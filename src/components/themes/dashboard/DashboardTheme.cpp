...
// v18.9.9.465 (P3d): 1:1 port of CrossInk v1.4.0 DashboardTheme.cpp with
// pragmatic adaptations documented in DashboardTheme.h.
...
constexpr int kStatsRowCount = 7;      // X3 (RTC-full)
constexpr int kStatsRowCountX4 = 6;    // X4 (no daily-avg row)
constexpr int kStatsValueLabelGap = 1;

// v18.9.9.465: X3 detection via HalGPIO singleton. isRtcCapable() gate for
// which stats rows to show — differs from upstream's rigid deviceIsX3() so
// X4 users with SNTP get the full row set (daily-avg etc.).
bool isRtcCapable() {
  // The RTC-derived rows (daily-avg, started/finish dates) all need
  // a valid clock. On X3 that's always true; on X4 it's true once SNTP
  // has synced. Match earlier v445 Minimal Stats decision.
  ReadingStatsDateTime dummy;
  return getCurrentLocalReadingStatsDateTime(dummy);
}
...
Rect coverRectForDashboard(const GfxRenderer& renderer, const Rect& rect) {
...
std::string coverPathForRect(const RecentBook& book, const Rect& imageRect) {
  ...
  // Dashboard runtime coverRect (~242x363 X4) rarely matches an existing thumb.
  // HomeActivity's cover-gen path writes at MinimalMetrics dims — look those up
  // first so we hit the file HomeActivity generated; drawBookCoverInto scales
  // to fit imageRect. Fall back to imageRect-sized (in case HomeActivity ever
  // gains a Dashboard-native gen path).
  if (FsHelpers::hasEpubExtension(book.path)) {
    ...
    if (Storage.exists(minimalPath.c_str())) return minimalPath;
    const std::string adaptivePath = epub.getAdaptiveThumbBmpPath(imageRect.width, imageRect.height);
    if (Storage.exists(adaptivePath.c_str())) return adaptivePath;
  }
  const std::string minimalGeneric = UITheme::getCoverThumbPath(
      book.coverBmpPath, MinimalMetrics::homeCoverImageWidth, MinimalMetrics::homeCoverImageHeight);
  if (Storage.exists(minimalGeneric.c_str())) return minimalGeneric;
...
void drawBookCoverInto(const GfxRenderer& renderer, const Rect& coverRect, const RecentBook& book,
  ...
  if (!hasCover) drawMissingBookCover(renderer, coverRect, book);
}
...
void drawDashboardStats(const GfxRenderer& renderer, const Rect& coverRect, const BookReadingStats* stats,
  ...
  const bool showRtcStats = isRtcCapable();  // CrumBLE: clock-valid gate, not deviceIsX3
  ...
  const bool hasEstimate = estimatedTimeLeftFor(bookStats, progressPercent, estimatedSeconds);
  ReadingStatsDateTime today;
  const bool hasToday = showRtcStats && getCurrentLocalReadingStatsDateTime(today);
  const ReadingStatsDate endDate = bookStats.isCompleted && bookStats.finishDate.isValid()
                                       ? bookStats.finishDate
  ...
  if (bookStats.isCompleted) {
    finishDisplayDate = bookStats.finishDate;
...
const char* readerTypeLabelFor(const GlobalReadingStats* globalStats) {
  if (globalStats == nullptr) return tr(STR_STATS_NEW_READER);
...
const uint8_t* readerTypeIconFor(const GlobalReadingStats* globalStats) {
  if (globalStats == nullptr) return Book24Icon;
  ReadingTimeBucket bucket = ReadingTimeBucket::Night;
...
void drawFooterStats(const GfxRenderer& renderer, const Rect& coverRect, const GlobalReadingStats* globalStats,
  ...
  const char* readerLabel = readerTypeLabelFor(globalStats);
  ...
  drawRightAlignedIconLabel(renderer, readerTypeIconFor(globalStats), rightX, centerY, readerLabel, maxReaderTextW,
                            inverted);
}

void drawBookText(const GfxRenderer& renderer, const Rect& coverRect, const RecentBook& book, const bool black = true) {
  const int inset = contentInset(renderer);
  ...
  // v18.9.9.202: subtitle prefers the current chapter title, persisted by
  // the reader as <cache>/chapter_title.txt on exit/deep-sleep. Falls back
  // to author when the sidecar is missing/empty (book never opened, or a
  // chapter with no ToC entry).
  std::string subtitle;
  if (FsHelpers::hasEpubExtension(book.path)) {
    const std::string sidecarPath =
        Epub::cachePathForFilePath(book.path, "/.crosspoint") + "/chapter_title.txt";
    FsFile f;
    if (Storage.openFileForRead("DASH", sidecarPath, f)) {
      char buf[121] = {};
      const int n = f.read(reinterpret_cast<uint8_t*>(buf), sizeof(buf) - 1);
      f.close();
      if (n > 0) subtitle.assign(buf, static_cast<size_t>(n));
    }
  }
  if (subtitle.empty()) subtitle = book.author;
  if (!subtitle.empty()) {
    auto subtitleLines = renderer.wrappedText(UI_12_FONT_ID, subtitle.c_str(), textW, 2);
...
void DashboardTheme::drawRecentBookCover(GfxRenderer& renderer, Rect rect,
  ...
  const Rect coverRect = coverRectForDashboard(renderer, rect);
  ...
  if (!coverRendered) {
    drawBookCoverInto(renderer, coverRect, recentBooks[0], Color::White);
    coverBufferStored = storeCoverBuffer();
    coverRendered = coverBufferStored;
  }

  // v18.9.9.465: load globalStats inline (Home doesn't pass it through).
  const GlobalReadingStats globalStats = GlobalReadingStats::load();
  drawDashboardStats(renderer, coverRect, stats, progressPercent);
  drawBookText(renderer, coverRect, recentBooks[0]);
...
void DashboardTheme::drawDashboardSleepScreen(const GfxRenderer& renderer, const RecentBook& book,
                                              ...
                                              const GlobalReadingStats& globalStats, const bool clockValid) const {
  (void)clockValid;  // isRtcCapable() re-checks internally
  renderer.clearScreen(0x00);  // BLACK background (inverted sleep)
  ...
  const Rect coverRect = coverRectForDashboard(renderer, contentRect);
  drawBookCoverInto(renderer, coverRect, book, Color::Black);
  drawDashboardStats(renderer, coverRect, stats, progressPercent, /*black=*/false);
  drawBookText(renderer, coverRect, book, /*black=*/false);
  drawFooterStats(renderer, coverRect, &globalStats, /*inverted=*/true);
}
