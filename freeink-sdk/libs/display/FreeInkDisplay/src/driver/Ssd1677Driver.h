...
class Ssd1677Driver : public PanelDriver {
 public:
  ...
  // CrumBLE 4.5.7: seamless silent-restart hook. HalDisplay::begin(seamless=true)
  // is called after ESP.restart() when the panel already shows the pre-restart
  // frame we want to keep. begin() arms _needsInitialFull to clear a physical
  // boot screen; on a silent restart there is nothing to clear, so promoting the
  // first paint to HALF/FULL is a visible flash. Upstream defines
  // skipInitialResync() on X3/UC8179/UC8279 but not here, and its PanelDriver
  // default is a no-op -- without this override the X4 flash comes back.
