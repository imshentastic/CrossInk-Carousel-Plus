...
class Ssd1677Driver : public PanelDriver {
 public:
  ...
  // frame we want to keep. The base display() path arms _needsInitialFull on
  // begin() to clear a physical black boot screen; on a silent restart there
  // is nothing to clear, so promoting the first paint to FULL is a visible
  // 3.7 s flash. Override skipInitialResync() to defuse the flag -- reuses the
  // existing X3 plumbing without a new virtual.
