...
### Added
- **New `tiny-bitter-chinese-fallback` firmware variant: Chinese reading with Bluetooth, no SD font required.** A separate build (`crumble-firmware-chinese-*.bin`, flashable to the stock 6.25 MB slot like the main build) bakes a Chinese font (LXGW WenKai, 8,846 hanzi -- GB2312 for simplified plus Big5 level 1 for traditional -- with hiragana/katakana, CJK punctuation and fullwidth forms) directly into firmware flash. Simplified and traditional both work with no setting to choose between them: the baked face is consulted by character, so a traditional book asks for 學 and a simplified one asks for 学, and a book mixing the two renders correctly on the same line. Glyphs render memory-mapped with zero heap and zero SD reads, so the BLE page-turner works while reading CJK books — the combination that the SD-card `.cpfont` path could never sustain on this hardware's heap budget. The baked face acts as an automatic fallback: keep Bitter selected, and any character Bitter lacks (hanzi, kana, fullwidth punctuation) renders from the CJK face, in books and in UI labels/titles alike. It is baked at 14pt, whose metrics match built-in Bitter 14 exactly, so hanzi sit at the same visual size as surrounding Latin and mixed lines need no extra leading. OPDS, Calibre, QR, and KOReader sync all remain included. Variant trade-offs: italic/bold-italic render as regular/bold, CJK renders at one fixed size (Latin keeps all sizes), and non-English hyphenation patterns are dropped (English kept; CJK needs none). OTA is variant-aware: a Chinese-fallback device only ever updates to `-cjk-` release assets, and the main firmware explicitly refuses them.
- **Chinese text is now set solid, without a space between every character** (`tiny-bitter-chinese-fallback`). The CJK tokenizer gives each ideograph its own word so lines can wrap between characters, which also made the line breakers insert a full space advance between every pair. Chinese is set without inter-character spacing, so that gap is now suppressed for CJK-to-CJK boundaries (a CJK/Latin boundary keeps its space, as is conventional). Besides reading correctly, this fits ~17% more characters per line, which proportionally cuts the number of pages — and therefore the time — a Chinese chapter takes to index.
...
### Fixed
- **Black boxes no longer stick to the screen, and covers no longer vanish, while moving around the home screen.** A change in 4.7.3 altered how the firmware waits for the screen to finish redrawing: instead of watching the panel's busy signal, it waited for a single electrical pulse from it. The panel emits more than one such pulse during a redraw, so the firmware could conclude a redraw was finished while it was still running, then immediately overwrite the panel's own record of what it was showing. From that point the firmware's idea of the screen and the actual screen disagreed, and because a fast redraw only touches pixels it believes are changing, whatever was wrong stayed wrong — a highlight box left behind under the icon row, covers that disappeared until you scrolled past them again, a "Loading" message that would not clear. The wait is back to watching the busy signal, and the pulse-based wait (unused by this device today) now confirms the signal really has settled before returning.
- **The icon row no longer leaves a trail of black squares as you move along it.** The focused icon is drawn as a black tile with the icon knocked out of it, but the row was never wiped before being redrawn — so the highlight was simply painted on top of whatever was already there. Home only wipes the whole screen when it can't reuse its saved copy of the cover area, which meant that most of the time the previous icon's black tile stayed put and a new one appeared beside it. The row now clears itself first, the same way the label beneath it already did.
- **Book covers no longer go blank on the carousel after switching themes.** Moving down off the carousel, or pressing left/right quickly, could leave an empty book frame — or a title that advanced while the cover under it stayed put. Home keeps a snapshot of part of the screen so it can put those pixels back instead of redrawing them, and the two halves of that mechanism disagreed about which part had been saved: the save went by the theme in use, the restore went by whichever screen area had been measured most recently. The shelf measurement is left behind when you leave the Flow theme, so on the Carousel theme afterwards the restore put the covers back in the wrong place and reported success — and the covers were then never redrawn. The restore now goes back exactly where the save came from, and falls back to a clean redraw if it can't.
- **The Bluetooth Debug Capture screen now updates as you press keys on the remote.** It counted every key event correctly and wrote them all to the serial log, but nothing ever asked the screen to redraw — so it painted once when you opened it and then sat there, looking as though the remote wasn't being seen at all. The screen now refreshes when new events arrive, at most a couple of times a second so that holding a key down doesn't leave the display running behind.
- **Bluetooth Debug Capture no longer crashes the device when you press a button on the remote.** Turning on Debug Capture and pressing keys reliably rebooted the reader. Each incoming key report was written to the log from inside Bluetooth's own handler, which runs with a deliberately small amount of working space; formatting those lines needed more than it had. Key reports are now recorded as they arrive and written to the log a moment later from the main program, which has room for it. Nothing is lost from the capture: if reports arrive faster than they can be written out, the log says how many were dropped.
...
- **Chinese text no longer stacks on top of itself, and indexing a Chinese chapter is far faster.** Measuring text through an SD-card font used a fast path that reports zero width for any character the font lacks, and skipped the fallback chain entirely. Every ideograph therefore measured as zero-width: the whole line drew at a single position, and the line breaker — which stops when a line overflows — never found an overflow, so it degraded to comparing every word against every other word. One page took over fourteen seconds. Both the width measurement and the line breaker now fall back correctly. Ported from upstream PR #2288, extended to a second measurement function carrying the same flaw.
- **"Use prepared layout?" no longer asks repeatedly during a single Bluetooth Quick Connect.** The Quick-Connect prompt decided whether it had already been answered using per-boot state alone, ignoring the cross-restart record that the other prompt site consults. Because the Bluetooth enable pre-flight silently restarts the device to defragment the heap — sometimes more than once — and each restart re-armed that per-boot flag, one Quick Connect could put the same question on screen three times in a row. It now honours the recorded answer, including a decline already given to the open-book prompt, so it asks at most once. Completes the v18.9.9.187 fix, which wired the same flag into the link-detect prompt but not this one.
