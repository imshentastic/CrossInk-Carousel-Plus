...
- **"Sort by" → Author no longer crashes the device on large collections** like All Books. The per-book `Epub::load` loop in `CollectionsStore::applySort`'s AuthorAlpha branch used to run without yielding and without per-book heap pre-flight, so on 200+ books it either tripped the IDLE-task watchdog (5 s default → panic + reboot) or OOM-terminated when a heavy book hit a tight-heap state. The loop now yields to FreeRTOS every 16 books and pre-flights maxAllocHeap before each load (skipping books that won't fit; they sort to the end like books with no author metadata). Other sort modes were already heap-light and unchanged.
